...
for numCells in [50]:
    ...
    for j in range(1):
        ...
        numROIXYSteps=9 #(number of roi+1)
        ...
        #install dask on a remote machine and run "dash scheduler" in one window and "dask worker IPHOST:8786" in the other
        #compute=DistributedComputeLocal() # this setting the worst case scenario
        #compute=DistributedComputeDaskTask() #this setting is multithreadeing localy
        ...
        apdFunction=lib.findNumCells(xROIRange,yROIRange,xyROIOrigin,ROIImSize,channels=['Channel',['Cy5', 'DAPI'],[100.,100.]],
laserIntensityRGBV=[6.,1.,5.,0.],zRange=None,timeRange=None,emulator=emulator,compute=compute,model_type='nuclei',calibration=ViewToStageXYCalibrator)

        dataset=apd.run(apdFunction)
        print('TrueNumCellsInROI={0}'.format(emulator.num_cells_in_image))
        #numCellsPerIter.append(dataset['cellDetectNumCellsInRoi']['numcells'])
        #print(dataset)
#emulator.saveCache()