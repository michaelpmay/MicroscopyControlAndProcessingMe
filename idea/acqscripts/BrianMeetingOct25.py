...
# connect to \\munsky-nas.engr.colostate.edu\share and map as Z drive  before running this line (map as z drive?)
userName = 'Brian'
userPwd = ''
saveName = 'ER_A549_030823' #!!!! Does not save with correct name!!!

#General Settings
showDisplayOption = True  # Show images as we take them or just save them to file?

#z-position determination scan settings
colors_zPosCalibration = ['Red'] # !!!Still calibrates in Red regardless of input!!!
...
# FOV Scan Settings
stepSize = 80
rangeX = range(0,stepSize*9+1,stepSize)
rangeY = range(0,stepSize*9+1,stepSize)
#zRange = [0]  # position of z-stacks relative to the center plane
zRange = np.linspace(-13*1/2., 9*1/2., 2*11+1)  # position of z-stacks relative to the center plane
exposureTimes_FOVs = [0,100,300] #Assumed order is [488nm, 561nm, 637nm]??
colors = ['Red','Green','Blue'] #opposite order with previous and next lines??
laserIntensities = [0., 1., 2.] #Again, assumed order is [488nm, 561nm, 637nm]??
