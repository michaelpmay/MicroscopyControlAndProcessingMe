...
class Cellpose():
    ...
    '''
    def __init__(self, image:np.ndarray, num_iterations:int = 6, channels:list = [0, 0], diameter:float = 120, model_type:str = 'cyto', selection_method:str = 'cellpose_max_cells_and_area', NUMBER_OF_CORES:int=1,cellprob_threshold=0):
        self.image = image
    ...
    def calculate_masks(self):
        ...
        def cellpose_max_area( optimization_parameter):
            try:
                masks= model.eval(self.image, batch_size=self.BATCH_SIZE, normalize = True, flow_threshold = optimization_parameter, diameter = self.diameter, min_size = self.MINIMUM_CELL_AREA, channels = self.channels, progress = None,cellprob_threshold=self.cellprob_threshold)[0]
                # removing artifacts.
...
class CellSegmentation():
    ...
    '''
    def __init__(self, image:np.ndarray, channels_with_cytosol = None, channels_with_nucleus= None, diameter_cytosol:float = 150, diameter_nucleus:float = 100, optimization_segmentation_method='default', remove_fragmented_cells:bool=False, show_plots: bool = False, image_name = None,NUMBER_OF_CORES=1, running_in_pipeline = False ):
        self.image = image
    ...
    def calculate_masks(self):
        ...
        def function_to_find_masks (image):                    
            if not (self.channels_with_cytosol in (None,[None])):
                masks_cyto = Cellpose(image.squeeze(),diameter = self.diameter_cytosol, model_type = 'cyto', selection_method = 'max_cells_and_area' ,NUMBER_OF_CORES=self.NUMBER_OF_CORES,cellprob_threshold=0).calculate_masks()
            else:
                masks_cyto = np.zeros_like(image[:, :, 0])
            if not (self.channels_with_nucleus in (None,[None])):
                masks_nuclei = Cellpose(image[:, :, self.channels_with_nucleus],  diameter = self.diameter_nucleus, model_type = 'nuclei', selection_method = 'max_cells_and_area',NUMBER_OF_CORES=self.NUMBER_OF_CORES,cellprob_threshold=0).calculate_masks()
            else:
...
class ReportPDF():
    ...
    def __init__(self, directory,filenames_for_pdf_report, channels_with_FISH,save_all_images,list_z_slices_per_image,threshold_for_spot_detection,list_segmentation_successful=True):
        ...
        if isinstance(channels_with_FISH, list): 
            ...
            self.channels_with_FISH = [channels_with_FISH]
        self.images = save_all_images
        self.save_all_images = self.images
            ...
            if self.list_is_image_sharp[i] == False: 
                ...
                # test if segmentation was succcesful
                    ...
                    if Utilities().is_None(self.channels_with_nucleus) == False: #not (self.channels_with_nucleus in (None,[None])) :
                        mask_nuc_path = self.masks_dir.absolute().joinpath('data','temp','masks_nuclei_' + temp_file_name +'.tif' )
                        try:
                    ...
                    if Utilities().is_None(self.channels_with_cytosol) ==False: #not (self.channels_with_cytosol is None):
                        mask_cyto_path = self.masks_dir.absolute().joinpath('data','temp', 'masks_cyto_' + temp_file_name +'.tif' )
                        try:
                    ...
                    if  (Utilities().is_None(self.channels_with_nucleus) == False) and (Utilities().is_None(self.channels_with_cytosol) ==False): # not (self.channels_with_cytosol is None) and not (self.channels_with_nucleus is None) :
                        mask_cyto_no_nuclei_path = self.masks_dir.absolute().joinpath('data','temp','masks_cyto_no_nuclei_' + temp_file_name +'.tif' )
                        try:
                ...
                if (self.save_masks_as_file ==True) and (segmentation_successful==True) :
                    ...
                    if Utilities().is_None(self.channels_with_nucleus) == False: #not (self.channels_with_nucleus is None):
                        mask_nuc_path = pathlib.Path().absolute().joinpath( 'data','temp',masks_folder_name, 'masks_nuclei_' + temp_file_name +'.tif' )
                        tifffile.imwrite(mask_nuc_path, masks_nuclei)
                    if Utilities().is_None(self.channels_with_cytosol) ==False: #not (self.channels_with_cytosol is None):
                        mask_cyto_path = pathlib.Path().absolute().joinpath('data','temp', masks_folder_name, 'masks_cyto_' + temp_file_name +'.tif' )
                ...
                if segmentation_successful==True:
                    temp_detection_img_name = pathlib.Path().absolute().joinpath( 'data','temp', 'det_' + temp_file_name )
...
class MicroscopeSimulation():
    ...
    def generate_simulated_positions (self,image_size_Y_X,number_of_cells_in_simulation,list_library_cells,dataframe_cell_library,generate_cells_close_to_each_other=True):
        ...
        while (counter< MAX_NUM_ITERATIONS-1) and (printed_cells<=large_number_initial_simulation-1):
            ...
            if add_cell == True: 
                ...
                list_ts = [dataframe_cell_library.loc[   (dataframe_cell_library['cell_id']==library_cell_index) ].ts_size_0.values ,
    ...
    def make_simulated_image(self, z_position, y_position, x_position, x_size, y_size, complete_image_size_Z_Y_X, simulation_dataframe, list_library_cells, background_pixels_library = None,alpha_0=0,alpha_1=0,alpha_2=0,remove_elements_low_intensity=False):
        ...
        #full_image_z_range = (0,complete_image_size_Z_Y_X[0])
